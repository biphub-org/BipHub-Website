...
- [ ] **FOUN-14**: Adding any BIP-model field updates all three seed sources (`seed.sql`, `seed.e2e.sql`, `seed-cloud-e2e.mjs`) and the duplicated `bip_edits` column-list literals are consolidated into one shared constant *(anti-drift; prevents the BUG-002-class regression)*
