...
- **D-26: License + CoC** (Claude's call): A LICENSE file already exists per FOUN-08 (now proprietary — all rights reserved). Adopt Contributor Covenant v2.1 verbatim as `CODE_OF_CONDUCT.md` (linked from CONTRIBUTING.md). Both files unchanged across edits — they're foundation docs.
...
- **Single admin in v1** — `biphub.org@gmail.com` per Phase 3 D-bootstrapping note. Privacy policy "Data Controller" section uses this address. Update if the production admin email changes before launch.
