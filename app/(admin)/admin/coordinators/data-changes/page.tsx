...
import { getAdminProfileChangeRequests } from '@/lib/queries/profileChangeRequests'
import { ProfileChangeRequestCard } from '@/components/admin/ProfileChangeRequestCard'

export const dynamic = 'force-dynamic'

/**
 * /admin/coordinators/data-changes — coordinator data-change inbox.
 *
 * Approving applies the requested values to the coordinator's profiles row
 * and emails them; declining leaves the profile untouched and emails the
 * admin note. Decided requests stay visible below as history.
 */
export default async function ProfileChangeRequestsPage() {
  const requests = await getAdminProfileChangeRequests()
  ...
  return (
    <div>
      <div className="border-b border-border bg-white px-6 py-5">
        ...
        <h1 className="text-[22px] font-semibold text-ink">Data-change requests</h1>
        ...
        <section aria-label="Pending requests">
          ...
          {pending.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {pending.map((r) => (
                <ProfileChangeRequestCard key={r.id} request={r} />
              ))}
            </div>
          )}
        </section>
